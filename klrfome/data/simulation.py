...
def get_sim_data(
    ...
    site_nos = np.random.choice(
        [f"Site{i}" for i in range(1, N_site_bags + 1)], size=site_samples, replace=True
    )
...
def sim_trend(
    ...
    for row in range(rows):
        for col in range(cols):
            ...
            distances = np.sqrt((site_coords[:, 0] - col) ** 2 + (site_coords[:, 1] - row) ** 2)
    ...
    return {"coords": site_coords, "trend": trend}


def nlm_gaussianfield(
    cols: int, rows: int, autocorr_range: float = 20.0, seed: Optional[int] = None
) -> np.ndarray:
    """
...
def rescale_sim_raster(raster_data: np.ndarray, mean: float, sd: float) -> np.ndarray:
    """
...
def create_simulated_raster_stack(
    cols: int = 100, rows: int = 100, n_bands: int = 2, seed: int = 42
) -> RasterStack:
    """
    ...
    return RasterStack(data=data_jax, transform=transform, crs="EPSG:4326", band_names=band_names)
