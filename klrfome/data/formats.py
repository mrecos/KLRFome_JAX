...
class Bag:
    ...
    coordinates: Optional[Float[Array, "n_samples 2"]] = None
    ...
    def __post_init__(self) -> None:
        ...
        if samples.ndim != 2 or samples.shape[0] == 0 or samples.shape[1] == 0:
            raise ValueError("Bag samples must be a nonempty 2D array")
        if not np.isfinite(np.asarray(samples)).all():
            raise ValueError("Bag samples must be finite")
        if self.label not in (0, 1):
            raise ValueError("Bag label must be 0 or 1")
        if not str(self.id):
            raise ValueError("Bag id must be nonempty")
        self.samples = samples
        self.id = str(self.id)
        if self.coordinates is not None:
            coordinates = jnp.asarray(self.coordinates)
            if coordinates.shape != (samples.shape[0], 2):
                raise ValueError("Bag coordinates must have one x/y pair per sample")
            if not np.isfinite(np.asarray(coordinates)).all():
                raise ValueError("Bag coordinates must be finite")
            self.coordinates = coordinates
        if self.group_id is not None:
            self.group_id = str(self.group_id)
        if self.stratum_id is not None:
            self.stratum_id = str(self.stratum_id)
...
class BagDataset:
    ...
    collections: List[Bag]
    feature_names: List[str]
    crs: Optional[str] = None
    study_design: Literal["presence_background", "presence_absence"] = "presence_background"
    metadata: Optional[Dict[str, Any]] = None

    def __post_init__(self) -> None:
        if not self.collections:
            raise ValueError("BagDataset must contain at least one bag")
        ...
        if self.study_design not in ("presence_background", "presence_absence"):
            raise ValueError("study_design must be presence_background or presence_absence")

        expected_features = len(self.feature_names)
        ids = [bag.id for bag in self.collections]
        if len(set(ids)) != len(ids):
            raise ValueError("Bag ids must be unique")
        for bag in self.collections:
            if bag.n_features != expected_features:
                raise ValueError(
                    f"Bag {bag.id!r} has {bag.n_features} features; expected {expected_features}"
                )
            bag_feature_names = (bag.metadata or {}).get("feature_names")
            if bag_feature_names is not None and list(bag_feature_names) != self.feature_names:
                raise ValueError(f"Bag {bag.id!r} has inconsistent feature order")
            bag_crs = (bag.metadata or {}).get("crs")
            if bag_crs is not None and self.crs is not None and str(bag_crs) != str(self.crs):
                raise ValueError(f"Bag {bag.id!r} has an inconsistent CRS")

    @property
    def bags(self) -> Tuple[Bag, ...]:
        """Immutable view used by new code; ``collections`` remains compatible."""
    ...
    def train_test_split(
        self, test_fraction: float = 0.2, stratify: bool = True, seed: int = 42
        ...
        if stratify:
            ...
            train_background = (
                background[n_background_test:] if n_background_test > 0 else background
            )
    ...
    def subset(self, indices: Sequence[int]) -> "BagDataset":
        """Return a metadata-preserving dataset subset without mutating bags."""
        return BagDataset(
            collections=[self.collections[index] for index in indices],
...
# Compatibility aliases.  They intentionally point to the canonical classes so
# old imports and constructor calls retain their behavior.
SampleCollection = Bag
TrainingData = BagDataset
...
class RasterStack:
    ...
    def from_files(cls, file_paths: List[str]) -> "RasterStack":
        ...
        return cls(data=data, transform=transform, crs=crs, band_names=band_names, nodata=nodata)
    ...
    def extract_window(self, row: int, col: int, size: int) -> Float[Array, "size size n_bands"]:
        """
        ...
        pad = size // 2
        padded = jnp.pad(self.data, ((0, 0), (pad, pad), (pad, pad)), mode="reflect")
    ...
    def extract_at_points(
        ...
        return extract_at_points(self, points, buffer_radius, n_samples, random_seed)

    def save(
        self, file_path: str, data: Optional[Float[Array, "height width"]] = None, band: int = 1
    ):
        """
