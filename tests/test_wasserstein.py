...
class TestSlicedWasserstein:
    ...
    def test_pairwise_distances_shape(self):
        ...
        collections = [
            SampleCollection(
                samples=random.normal(random.PRNGKey(i), (10, 4)), label=i % 2, id=f"coll_{i}"
            )
            for i in range(5)
        ]
...
class TestWassersteinKernel:
    ...
    def test_similarity_matrix_shape(self):
        ...
        collections = [
            SampleCollection(
                samples=random.normal(random.PRNGKey(i), (15, 4)), label=i % 2, id=f"coll_{i}"
            )
            for i in range(10)
        ]
    ...
    def test_compute_new_to_training(self):
        ...
        training = [
            SampleCollection(
                samples=random.normal(random.PRNGKey(i), (15, 4)), label=i % 2, id=f"train_{i}"
            )
            for i in range(5)
        ]
...
class TestIntegration:
    ...
    def test_full_workflow(self):
        ...
        for i in range(n_sites):
            ...
            collections.append(SampleCollection(samples=samples, label=1, id=f"site_{i}"))

        # Background: samples from shifted distribution
        for i in range(n_background):
            samples = (
                random.normal(random.PRNGKey(100 + i), (samples_per_collection, n_features)) + 2.0
            )
            collections.append(SampleCollection(samples=samples, label=0, id=f"bg_{i}"))
...
class TestBucketing:
    ...
    def test_bucketing_ceiling_vs_median(self):
        ...
        for size in [25, 30, 35, 40]:
            ...
            collections.append(SampleCollection(samples=samples, label=1, id=f"coll_{size}"))
            key, _ = random.split(key)

        distance_fn = SlicedWassersteinDistance(n_projections=10, seed=42)

        # Test ceiling mode (should target 40)
        K_ceil = distance_fn.pairwise_distances(collections, bucket_width=50, bucket_ceil=True)

        # Test median mode (should target 32 or 33)
        K_median = distance_fn.pairwise_distances(collections, bucket_width=50, bucket_ceil=False)
    ...
    def test_global_bucket_cap(self):
        ...
        key = random.PRNGKey(42)
        collections = [
            SampleCollection(samples=random.normal(key, shape=(100, 3)), label=1, id="small"),
            SampleCollection(
                samples=random.normal(random.PRNGKey(123), shape=(5000, 3)), label=1, id="large"
            ),
        ]
        ...
        K = distance_fn.pairwise_distances(collections, bucket_width=25, bucket_cap=2500)
    ...
    def test_backward_compatibility_bucket_tolerance(self):
        ...
        K = distance_fn.pairwise_distances(collections, bucket_tolerance=10)
    ...
    def test_wasserstein_kernel_with_bucketing(self):
        ...
        for i, size in enumerate([25, 30, 50, 75, 100]):
            samples = random.normal(key, shape=(size, 3))
            collections.append(
                SampleCollection(samples=samples, label=i % 2, id=f"coll_{i}")  # Alternate labels
            )
            key, _ = random.split(key)

        kernel = WassersteinKernel(sigma=1.0, n_projections=100, seed=42)
        K = kernel.build_similarity_matrix(
            collections, bucket_width=25, bucket_ceil=True, bucket_cap=200
        )
