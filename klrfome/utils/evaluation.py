...
def paired_method_differences(
    ...
    """Summarize paired fold differences against a baseline method."""
    lookup = {(str(row["method"]), int(row["repeat"]), int(row["fold"])): row for row in rows}
    ...
    for method in methods:
        ...
        for metric in metrics:
            ...
            for key, row in lookup.items():
                if key[0] != method:
                    continue
                reference = lookup.get((baseline, key[1], key[2]))
