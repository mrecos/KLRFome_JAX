...
from klrfome.data.formats import SampleCollection, RasterStack
...
def test_sample_collection():
    ...
    coll = SampleCollection(samples=samples, label=1, id="test_site")
...
def test_training_data_split(training_data):
    ...
    train_data, test_data = training_data.train_test_split(
        test_fraction=0.4, stratify=True, seed=42
    )
...
def test_raster_stack_properties():
    ...
    stack = RasterStack(data=data, transform=transform, crs=crs, band_names=band_names)
